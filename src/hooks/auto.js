...
export default defineHook(async ({init, action}, {services, getSchema, logger}) => {
    ...
    const shouldIncludeRights = helpers.getEnvConfig().AUTOSYNC_INCLUDE_RIGHTS;
    ...
    async function doPush() {
        try {
            await snapshot.pushSnapshot(services, schema, accountability, false, versionData.version);
            if(shouldIncludeRights) {
                await rights.pushRights(services, schema, accountability, false, versionData.version);
            }
        } catch (e) {
    ...
    async function doPull() {
        try {
            await snapshot.pullSyncFiles(services, schema, accountability, versionData.version);
        } catch (e) {
